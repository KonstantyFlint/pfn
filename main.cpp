...
double distance_to_segment(point2d v, point2d w, point2d p) {
    const float l2 = distance(v, w) * distance(v, w);
...
int main(int argc, char **argv) {
    ...
    /*for (double x = 4.6; x < 5.7; x += 0.01) {
        for (double y = -3.6; y < 3.7; y += 0.01) {
            obstacles.push_back({{x, y}, (argc > 1) ? std::stod(argv[1]) : 0.1});
        }
    }*/
...
//    currentPosition
    ...
    for (int i = 0; i < 1000; i++) {
        ...
        if (length(currentVelocity) > velocity)
            currentVelocity =
                    (currentVelocity * (1.0 / (length(currentVelocity)))) *
                    velocity;
